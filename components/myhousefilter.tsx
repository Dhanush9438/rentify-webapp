...
const MyHouseFilter = ({ houseData }: { houseData: HouseZodSchemaType[] }) => {
  ...
  const bhkOptions = Array.from(new Set(houseData.map((p) => p.bedrooms))).sort();
  const allAmenities = Array.from(new Set(houseData.flatMap((p) => p.amenities.split(',').map(a => a.trim()))));
  ...
  const handleAmenityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedAmenities.includes(amenity)) {
      setSelectedAmenities(selectedAmenities.filter((el) => el !== amenity));
    } else {
    ...
    if (selectedAmenities.length > 0) {
      tempProperties = tempProperties.filter((house) =>
        selectedAmenities.every((amenity) =>
          house.amenities.split(',').map(a => a.trim()).includes(amenity)
        )
      );
    }
          ...
          <div className="w-[50%] ">
            <div className="mb-4">
              ...
              <select
                ...
                className="w-full p-2 border rounded bg-white text-black border-gray-600"
              >
            ...
            <div className="w-full flex ">
              <Button
                ...
                className="w-full px-4 p-4 bg-black text-white rounded-full hover:bg-gray-700"
              >
                Clear Filters
              </Button>
            </div>
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full ${
                  currentPage === 1
                    ...
                    : "bg-white text-black border border-gray-600 hover:bg-gray-200"
                }`}
